...
def get_weather_data(station_date_pairs):
    try:
        ...
        # SQL sorgusu
        query = """
            SELECT station_name, date, tageswetter
            FROM wetterdaten_zukunft
            WHERE (station_name, date) IN %s
        """
        # Parametreleri tuple formatına çevir
        ...
        weather_dict = {(row["station_name"], str(row["date"])): row["tageswetter"] for _, row in result.iterrows()}
        return weather_dict
            ...
            for item in route:
                ...
                try:
                    ...
                    try:
                        ...
                        # GMT başarısızsa, diğer formatları dene
                        try:
                            departure_dt = datetime.strptime(
                                item['planned_arrival_date_from'] + " " + item['planned_departure_from'],
                                "%Y-%m-%d %H:%M:%S"
                ...
                # arrival_time'ı departure_date ile birleştir
                ...
                # Gerekli input'ların JSON'da olduğundan emin ol
